#ifndef CAFFE_BASE_DEFORMABLE_CONVOLUTION_LAYER_HPP_
#define CAFFE_BASE_DEFORMABLE_CONVOLUTION_LAYER_HPP_
...
class BaseDeformableConvolutionLayer: public Layer<Dtype> {
public:
    explicit BaseDeformableConvolutionLayer(const LayerParameter& param) :
            Layer<Dtype>(param), is_deformable_conv_(false) {
    }
    ...
    inline void conv_im2col_gpu(const Dtype* data, Dtype* col_buff) {
        if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
            ...
            im2col_nd_gpu(data, num_spatial_axes_, num_kernels_im2col_,
                    conv_input_shape_.gpu_data(), col_buffer_.gpu_shape(), kernel_shape_.gpu_data(),
                    pad_.gpu_data(), stride_.gpu_data(), dilation_.gpu_data(), col_buff);
        }
    }
    inline void conv_col2im_gpu(const Dtype* col_buff, Dtype* data) {
        if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
            ...
            col2im_nd_gpu(col_buff, num_spatial_axes_, num_kernels_col2im_,
                    conv_input_shape_.gpu_data(), col_buffer_.gpu_shape(), kernel_shape_.gpu_data(),
                    pad_.gpu_data(), stride_.gpu_data(), dilation_.gpu_data(), data);
        }
    }
#endif
...
#endif  // CAFFE_BASE_DEFORMABLE_CONVOLUTION_LAYER_HPP_

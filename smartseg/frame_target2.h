...
public:
    ...
    void init(FrameTargetParameter param, Frame* frame, PlanView* view, Segmentor* segmentor) {
        ...
        _max_points = Config::instance().max_points();
        _rows = _view->rows();
        _cols = _view->cols();
        _grids = _view->grids();
    }
    void reshape() {
        _objectness_target_blob->Reshape(1, 1, _max_points, 1);
        _objectness_weight_blob->Reshape(1, 1, _max_points, 1);
...
private:
    ...
    std::vector<std::vector<int>> _obstacle_points;
    ...
    void init_obstacle(const ObstacleLabel& obstacle, int obs_id) {
        const LidarPoint* cloud = _frame->cloud.data();
        thread_local std::vector<int> grid_list;
        thread_local std::vector<int> point_list;
        grid_list.clear();
        point_list.clear();
        ...
        _obstacle_grids[obs_id] = grid_list;
        _obstacle_points[obs_id] = point_list;
    }
    void init_obstacles() {
        const std::vector<ObstacleLabel>& obstacles = _frame->obstacles;
        _obstacle_grids.resize(obstacles.size());
        _obstacle_points.resize(obstacles.size());
        for (int i = 0; i < (int)obstacles.size(); i++) {
            init_obstacle(obstacles[i], i);
        }
    }
    void compute_objectness_target() {
        ...
        caffe::caffe_set(_max_points, 0.0f, target_data);
        caffe::caffe_set(_max_points, 0.0f, weight_data);
        std::vector<float> point_weights(_max_points, 0.0f);

        for (int i = 0; i < (int)obstacles.size(); i++) {
            float weight = 1.0 / _obstacle_points[i].size();
            for (int pt : _obstacle_points[i]) {
                point_weights[pt] += weight;
                target_data[pt] = 1;
            }
        }
        ...
        for (int i = 0; i < (int)segments.size(); i++) {
            double tot_point_weight = 0;
            double tot_intersect = 0;
            double tot_union = 0;
            for (int grid : segments[i].potential_grids) {
                for (int pt : (*_view)(grid)) {
                    if (target_data[pt] > 0) {
                        tot_point_weight += point_weights[pt];
                        tot_intersect += prob_data[pt];
                        tot_union++;
                    } else {
                        tot_union += prob_data[pt];
                    }
                }
            }
            if (tot_point_weight > 0) {
                for (int grid : segments[i].potential_grids) {
                    for (int pt : (*_view)(grid)) {
                        if (target_data[pt] > 0 || equal_objectness_weight) {
                            weight_data[pt] = tot_point_weight / tot_union;
                        } else {
                            weight_data[pt] = tot_point_weight * tot_intersect / (tot_union * tot_union);
                        }
                    }
                }
            }
...
typedef FrameTarget2 FrameTarget;
